...
func TestImplementation_GetEventsBeforeDays(t *testing.T) {
	t.Parallel()
	...
	}{
		{
			...
			fields: fields{
				eventServiceMockFunc: func(t *testing.T) eventService {
					...
					s.EXPECT().
						GetEventsBeforeDays(mock.Anything, mock.Anything).
						Return([]*event.Event{
							{
								ID:        1,
								CreatedAt: now,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := i.GetEventsBeforeDays(tt.args.ctx, tt.args.req)
